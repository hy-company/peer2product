<?php
  ...
  $shop = new functions;
  // get settings
  $SET = $shop->merge_objects($shop->get_json($SET['data/'].'settings.def'), $shop->get_json($SET['data/'].'settings.json'));
  // merge translation default and customizations
  $STR = $shop->merge_objects($shop->get_json($SET['data/'].'translation.def'),$shop->get_json($SET['data/'].'translation.json'));
?>
